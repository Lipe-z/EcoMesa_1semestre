...
def criar_doacao():
    ...
    while True:
        data_doacao = input(f"{bcolors.CYAN}\nDigite a data em que a doação foi postada (formato YYYY-MM-DD): {bcolors.ENDC}")
        try:
    ...
    while True:   
        while True:
            descricao = input(f"{bcolors.CYAN}\nDigite a descrição do item a ser doado: {bcolors.ENDC}")
            if descricao.strip():
                break
            else:
        ...
        while True:
                quantidade = input(f"{bcolors.CYAN}\nDigite a quantidade do item a ser doado: {bcolors.ENDC}")
                if quantidade.strip():
                    break
                else:
        ...
        while True:
            validade = input(f"{bcolors.CYAN}\nDigite a data de validade do item (formato YYYY-MM-DD): {bcolors.ENDC}")
            try:
        ...
        while True:
            continuar = input(f"{bcolors.CYAN}\nDeseja adicionar outro item? (s/n): {bcolors.ENDC}").strip().lower()
            if continuar in ["s", "n"]:
                break
            else:
...
def atualizar_doacao():
    ...
    while True:
        try:
            atualizar_data = input(f"{bcolors.CYAN}\nDigite a nova data da doação (formato YYYY-MM-DD, deixe em branco para manter a atual): {bcolors.ENDC}")
    ...
    while True:
        try:
            atualizar_itens = input(f"{bcolors.CYAN}\nDeseja atualizar os itens da doação? (s/n): {bcolors.ENDC}").strip().lower()
    ...
    if atualizar_itens == "s":
        while True:
            ...
            while True:
                try:
                    escolha_item = int(input(f"{bcolors.CYAN}\nQual item deseja atualizar? (digite o ID): {bcolors.ENDC}"))
                    break
                except ValueError:
                    print(f"{bcolors.RED}❌ ID inválido. Digite um número inteiro.{bcolors.ENDC}")
            ...
            while True:
                    descricao = input(f"{bcolors.CYAN}\nDigite a descrição do item a ser doado{bcolors.ENDC} {bcolors.YELLOW}(deixe em branco para manter a atual): {bcolors.ENDC}")
                    if descricao.strip() or descricao == "":
                        break

            while True:
                    quantidade = input(f"{bcolors.CYAN}\nDigite a quantidade do item a ser doado{bcolors.ENDC} {bcolors.YELLOW}(deixe em branco para manter a atual): {bcolors.ENDC}")
                    if quantidade.strip() or quantidade == "":
                        break

            while True:
                validade = input(f"{bcolors.CYAN}\nDigite a data de validade do item (formato YYYY-MM-DD){bcolors.ENDC} {bcolors.YELLOW}(deixe em branco para manter a atual): {bcolors.ENDC}")
                if validade.strip() == "":
                    break
                try:
                    datetime.strptime(validade, "%Y-%m-%d")
            ...
            while True:
                continuar = input(f"{bcolors.CYAN}\nDeseja atualizar outro item? (s/n): {bcolors.ENDC}").strip().lower()
                if continuar in ["s", "n"]:
                    break
                else:
                    print(f"{bcolors.RED}❌ Opção inválida. Digite s ou n.{bcolors.ENDC}")
...
def excluir_doacao():
    ...
    while True:
        try:
            id_doacao = int(input(f"\n{bcolors.CYAN}\nDigite o ID da doação que deseja excluir: {bcolors.ENDC}"))
            break
        except ValueError:
            print(f"{bcolors.RED}❌ ID inválido. Digite um número inteiro.{bcolors.ENDC}")
...
def menu():
    opcoes = {
        "1": ("Criar doação com itens", criar_doacao),
        "2": ("Listar doações completas", listar_doacoes),
        "3": ("Atualizar doação e itens", atualizar_doacao),
        "4": ("Excluir doação", excluir_doacao),
    }

    while True:
        print("\n=== MENU DOAÇÕES ===")
        for codigo, (descricao, _) in opcoes.items():
            print(f"{bcolors.GREEN}{codigo} - {descricao}{bcolors.ENDC}")
